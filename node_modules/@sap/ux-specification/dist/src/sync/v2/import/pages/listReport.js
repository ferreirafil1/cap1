 ...
 */
function addSettings(listReportConfig, manifest, jsonSchema, factory, pageKey, logger) {
    ...
    if (tableType === 'TreeTable' || tableType === 'AnalyticalTable' || tableType === 'GridTable') {
        localJsonSchema = jsonSchema['definitions'][tableType + '<LineItems>'];
    }
    else {
    ...
    //Filterbar:
    listReportConfig.filterBar = factory.createInstance(common_1.PageType.ListReport, 'FilterBar', listReportConfig.filterBar);
...
function createListReportConfig(manifest, flex, appSchema, fragments, logger) {
    ...
    const pageKeys = [];
    const v2Page = utils_1.findListReportPage(manifest[common_1.ManifestSection.generic].pages, pageKeys);
    if (!v2Page) {
        extensionLogger_1.log(logger, {
            severity: "error" /* Error */,

...
function getXsuaaServiceCredentials(token) {
    return typeof token === 'string'
        ? selectXsuaaInstance(jwt_1.decodeJwt(token))
...
function extractClientCredentials(serviceCreds) {
    if (!serviceCreds.clientsecret) {
        throw new Error("Cloud not extract client secret for clientId: " + serviceCreds.clientid + ".");
    }
    return {
...
function selectXsuaaInstanceWithoutJwt(xsuaaCredentials) {
    if (!arrayHasExactlyOneElement(xsuaaCredentials)) {
        logger.warn("The following XSUAA instances are bound: " + xsuaaCredentials.map(function (x) { return '\n\t- {x.xsappname}'; }) + "\n      No JWT given to select XSUAA instance. " + choseFirstOneText(xsuaaCredentials));
        return xsuaaCredentials[0];
    }
    return handleOneXsuuaInstance(xsuaaCredentials);
}
function selectXsuaaInstanceWithJwt(xsuaaCredentials, jwt) {
    var selectedAll = __spreadArray(__spreadArray([], matchingClientId(xsuaaCredentials, jwt)), matchingAudience(xsuaaCredentials, jwt));
    var selectedUnique = util_1.unique(selectedAll.map(function (x) { return x.clientid; })).map(function (id) { return selectedAll.find(function (y) { return y.clientid === id; }); });
    ...
    if (selectedUnique.length > 1) {
        logger.warn("Multiple XSUAA instances could be matched to the given JWT: " + xsuaaCredentials.map(function (x) { return "\n\t- " + x.xsappname; }) + "\n      " + choseFirstOneText(xsuaaCredentials));
        return selectedUnique[0];
    }
    if (!arrayHasExactlyOneElement(xsuaaCredentials)) {
        logger.warn("Multiple XSUAA instances found: " + xsuaaCredentials.map(function (x) { return "\n\t- " + x.xsappname; }) + "\n      None of those match either client id or audience from the given JWT. " + choseFirstOneText(xsuaaCredentials));
        return xsuaaCredentials[0];
    }
    return handleOneXsuuaInstance(xsuaaCredentials);
}
function choseFirstOneText(xsuaaCredentials) {
    return "Choosing the first one (xsappname: \"" + util_1.first(xsuaaCredentials).xsappname + "\").";
}
...
function matchingAudience(xsuaaCredentials, token) {
    return xsuaaCredentials.filter(function (credentials) {
        return jwt_1.audiences(token).has(credentials.xsappname);
    });
}
/**

...
function mergeSchemas(schemas: Schema[]): Schema {
	...
	const associations = schemas.reduce((associations: Association[], schema) => {
		return associations.concat(schema.associations);
	}, []);
	const entitySets = schemas.reduce((entitySets: EntitySet[], schema) => {
		return entitySets.concat(schema.entitySets);
	}, []);
	const entityTypes = schemas.reduce((entityTypes: EntityType[], schema) => {
		return entityTypes.concat(schema.entityTypes);
	}, []);
	const actions = schemas.reduce((actions: Action[], schema) => {
		return actions.concat(schema.actions);
	}, []);
	const complexTypes = schemas.reduce((complexTypes: ComplexType[], schema) => {

    ...
    for (const v2PageName in v2Pages) {
        ...
        if (jsonSchema) {
            if (page.pageType === common_1.PageType.ListReport) {
                page.config = listReport_1.createListReportConfig(manifest, flex, schemas[jsonSchema], fragments);
            }
            else if (page.pageType === common_1.PageType.AnalyticalListPage) {
                page.config = analyticalListPage_1.createAnalyticalListPageConfig(manifest, flex, schemas[jsonSchema], fragments);
            }
            else {
                page.config = objectPage_1.createObjectPageConfig(manifest, flex, schemas[jsonSchema], page.entitySet, fragments, logger);
            }
        }
        if (v2Page.pages) {
            page.navigation = {};

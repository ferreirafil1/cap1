...
function deleteColumnFromFragment(manifestSection, pattern, appId, fragments) {
    if (fragments) {
        ...
        let fragmentName = fragmentParts[fragmentParts.length - 1] + '.fragment.' + fragmentType;
...
function exportToFragment(exportResults, manifestPath, config, appId, viewTemplateType, logger) {
    ...
    const cellFragmentFileName = cellTemplateType ? cellTemplateType + '.fragment.' + fragmentType : null;
    if (fragments) {

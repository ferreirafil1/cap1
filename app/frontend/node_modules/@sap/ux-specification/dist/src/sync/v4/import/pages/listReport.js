 ...
 */
function addSettings(config, manifest, jsonSchema, factory, page, pageType, logger) {
    // Tranfer page level settings with sync rules
    common_3.transferSettingsOfObject(config, manifest, jsonSchema, page.id, []);
    // Check table settings from target structure
    config.table = factory.createInstance(pageType, 'Table');
    common_3.transferSettingsOfObject(config.table, manifest, jsonSchema['definitions']['Table'], page.id, []);
    config.table.toolBar = factory.createInstance(pageType, 'ToolBar', { actions: {} });
    common_3.transferSettingsOfObject(config.table.toolBar, manifest, jsonSchema['definitions']['ToolBar<LineItems>'], page.id, []);
    if (jsonSchema['definitions']['Actions<LineItems>']) {
        for (const actionKey in jsonSchema['definitions']['Actions<LineItems>'].properties) {
            ...
            common_3.transferSettingsOfObject(config.table.toolBar.actions[actionKey], manifest, jsonSchema['definitions'][`ToolBarAction<${action}>`], page.id, [actionKey]);
    ...
    common_3.transferSettingsOfObject(config.filterBar, manifest, jsonSchema['definitions']['FilterBar'], page.id, []);
    //transfer columns
    config.table.columns = {};
    if (jsonSchema['definitions']['LineItems']) {
        for (const columnKey in jsonSchema['definitions']['LineItems'].properties) {
            const column = jsonSchema['definitions']['LineItems'].properties[columnKey];
            if (!column.template) {
                config.table.columns[columnKey] = factory.createInstance(pageType, 'TableColumn');
                common_3.transferSettingsOfObject(config.table.columns[columnKey], manifest, jsonSchema['definitions']['TableColumn'], page.id, [columnKey]);
...
function createListReportConfig(manifest, jsonSchema, logger) {
    ...
    Object.keys(v4Pages).forEach((key) => {
        ...
        if ('name' in v4Page && common_2.getPageTypeV4(v4Page.name) === common_1.PageTypeV4.ListReport) {
            addSettings(config, manifest, jsonSchema, factory, v4Page, common_1.PageTypeV4.ListReport, logger);
            return;
        }
    });
    return config;

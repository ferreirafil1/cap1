 ...
 * @param ui5Version - SAPUI5 version
 * @param propertyDefinition - schema definiton of a particular property
 * @param nextDefinition - schema definition referred by $ref
 */
function evaluateExportRule(syncRule, baseId, breadcrumbs, key, configObject, exportResults, fullManifest, ids, title, pageName, ui5Version, propertyDefinition, nextDefinition) {
    // handle flex change
    if (syncRule.flex) {
        const flexChange = common_1.createFlexChange(configObject, breadcrumbs, syncRule, ui5Version, baseId, ids, title, key);
        exportResults.flexChanges.push(syncRule.flex.createFlexChange(flexChange, fullManifest));
    }
        ...
        if (ids[1] === 'sections') {
            //header sections
            targetAnnotation = targetAnnotationEncoded = title
 ...
 */
function transferManifestEntriesAndFlexChange(fullManifest, configObject, parentIds, stableIdParts, breadcrumbs, exportResults, appId, baseId, pageName, pageType, jsonSchema, schemaDefinition, ui5Version, factory, title) {
    ...
    if (schemaDefinition && schemaDefinition['properties']) {
        for (const key in schemaDefinition['properties']) {
            ...
            const nextDefinition = utils_1.getNextTargetDefinition(jsonSchema, title, currentConfigObject, propertyDefinition, key, factory, pageType);
            if (syncRule) {
                evaluateExportRule(syncRule, baseId, breadcrumbs, key, configObject, exportResults, fullManifest, ids, title, pageName, ui5Version, propertyDefinition, nextDefinition);
            }
            if (typeof currentConfigObject === 'object' && propertyDefinition) {
                ...
                for (const innerObject of objects) {
                    ...
                    transferManifestEntriesAndFlexChange(fullManifest, innerObject, [...ids, ...additionalIds], localIdParts, [...breadcrumbs, key], exportResults, appId, baseId, pageName, pageType, jsonSchema, nextDefinition.targetDefinition, ui5Version, factory, nextDefinition.title);
                }
            }
        }
    }
...
const exportObjectPage = (appId, objectPageConfig, manifest, pageName, jsonSchema, ui5Version) => {
    ...
    const objectPage = factory.createPageInstance(v4_1.PageType.ObjectPage, objectPageConfig);
    ...
    [], // no stableIds
    [], // no breadcrumbs
    exportResults, appId, baseId, pageName, v4_1.PageType.ObjectPage, jsonSchema, jsonSchema, //starting point for target definition
    ui5Version, factory);
 ...
 */
const exportListPage = (factory, appId, config, manifest, pageName, pageType, jsonSchema, ui5Version) => {
    var _a;
    const exportResults = defaultExportResult;
    exportResults.manifest = JSON.parse(JSON.stringify(manifest));
    ...
    if ((_a = config.table.toolBar) === null || _a === void 0 ? void 0 : _a.actions) {
        for (const actionKey in jsonSchema['definitions']['Actions<LineItems>'].properties) {
    ...
    const baseId = `${appId}::${pageLayoutInformation.id}::${pageName[0]}--`;
    if (!exportResults.manifest[v4_1.ManifestSection.ui5]['routing']['targets'][pageName[0]]) {
        const newV4Page = getV4ManifestPageTemplate(pageLayoutInformation.pageId, pageName[0]);
        exportResults.manifest[v4_1.ManifestSection.ui5]['routing']['targets'][pageName[0]] = newV4Page;
    }
    transferManifestEntriesAndFlexChange(exportResults.manifest, config, [], //no parentIds
    [], // no stableIds
    [], // no breadcrumbs
    exportResults, appId, baseId, pageName, v4_1.PageType.ListReport, jsonSchema, jsonSchema, //starting point for target definition
    ui5Version, factory);
    return exportResults;
};
/**
...
const exportListReportPage = (appId, listReportConfig, manifest, pageName, jsonSchema, ui5Version) => {
    const factory = new factory_1.MetadataInstanceFactoryV4();
    const listReport = factory.createPageInstance(v4_1.PageType.ListReport, listReportConfig);
    return exportListPage(factory, appId, listReport, manifest, pageName, v4_1.PageTypeV4.ListReport, jsonSchema, ui5Version);
};
/**
...
const exportAnalyticalListPage = (appId, analyticalListPageConfig, manifest, pageName, jsonSchema, ui5Version) => {
    const factory = new factory_1.MetadataInstanceFactoryV4();
    const analyticalListPage = factory.createPageInstance(v4_1.PageType.AnalyticalListPage, analyticalListPageConfig);
    return exportListPage(factory, appId, analyticalListPage, manifest, pageName, v4_1.PageTypeV4.AnalyticalListPage, jsonSchema, ui5Version);
};
/**
...
exports.exportPageV4 = (exportParametersV4, ui5Version) => {
    if (exportParametersV4[v4_1.SchemaType.ListReport]) {
        const { appId, page, manifest, jsonSchema } = exportParametersV4[v4_1.SchemaType.ListReport];
        ...
        const { appId, page, manifest, jsonSchema } = exportParametersV4[v4_1.SchemaType.ObjectPage];
        ...
        const { appId, page, manifest, jsonSchema } = exportParametersV4[v4_1.SchemaType.AnalyticalListPage];

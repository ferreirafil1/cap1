...
function exportConfig(exportParameters) {
    ...
    if (exportParameters[page_1.FioriElementsVersion.v2]) {
        ...
        exportResults = export_1.exportPageV2(v2Params, ui5Config);
    }
        ...
        exportResults = export_2.exportPageV4(v4Params, ui5Config);
    }
...
function importConfig(importParameters) {
    ...
    if (importParameters[page_1.FioriElementsVersion.v2]) {
        ...
        if (v2Params[schemaAccess_1.SchemaType.ListReport]) {
            const { manifest, flex, jsonSchema, fragments, logger } = v2Params[schemaAccess_1.SchemaType.ListReport];
            schema = import_1.createListReportConfig(manifest, flex, jsonSchema, fragments, logger);
        }
        else if (v2Params[schemaAccess_1.SchemaType.ObjectPage]) {
            const { manifest, flex, jsonSchema, entitySet, fragments, logger } = v2Params[schemaAccess_1.SchemaType.ObjectPage];
            ...
            const { manifest, flex, jsonSchema, fragments, logger } = v2Params[schemaAccess_1.SchemaType.AnalyticalListPage];
        ...
        if (v4Params[schemaAccess_1.SchemaType.ListReport]) {
            const { manifest, jsonSchema, logger } = v4Params[schemaAccess_1.SchemaType.ListReport];
            schema = import_2.createListReportConfig(manifest, jsonSchema, logger);
        }
        else if (v4Params[schemaAccess_1.SchemaType.ObjectPage]) {
            const { manifest, jsonSchema, routingId, logger } = v4Params[schemaAccess_1.SchemaType.ObjectPage];
            schema = import_2.createObjectPageConfig(manifest, jsonSchema, routingId, logger);
        }
        else if (v4Params[schemaAccess_1.SchemaType.AnalyticalListPage]) {
            const { manifest, jsonSchema, logger } = v4Params[schemaAccess_1.SchemaType.AnalyticalListPage];
            schema = import_2.createAnalyticalListPageConfig(manifest, jsonSchema, logger);
        }
    }
    return schema;
}
